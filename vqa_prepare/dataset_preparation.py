# modified from https://github.com/Holipori/EKAID/blob/main/model/dataset_preparation.py
...
def save_h5(questions, answers, pos, label_start_idx, label_end_idx, feature_idx,max_seq = 60, times=0, length = 100):
    filename = os.path.join('./vqa_prepare/data/VQA_mimic_dataset.h5')
    if times == 0:
...
def save_coco_format():
    ...
    path_splits = './vqa_prepare/data/splits_mimic_VQA.json'
    with open(path_splits, 'r')as f:
        splits = json.load(f)

    path = './vqa_prepare/data/mimic_pair_questions.csv'
    df = pd.read_csv(path)
    anno_list= []
    image_list = []
    for name in ['train','val','test']:
        ...
        json.dump(dict, open('./vqa_prepare/data/mimic_gt_captions_%s.json'%name, 'w'))
        image_list = []
        anno_list = []
        print('saved')
def transform_questions2dataset(simple = False, custom_dataset = False):
    ...
    question_path = './vqa_prepare/data/mimic_pair_questions.csv'
    d = pd.read_csv(question_path)
    pos_path = './vqa_prepare/data/POS.csv'
    d_pos = pd.read_csv(pos_path)
    with open('./vqa_prepare/data/dicom2id.pkl', 'rb') as f:
        dicom2id = pickle.load(f) # index in bbx features
    with open('./vqa_prepare/data/study2dicom.pkl', 'rb') as f:
        study2dicom = pickle.load(f)

    if custom_dataset:
        vocab = {'<start>':1}
    else:
        with open('./vqa_prepare/data/vocab_mimic_VQA.json', 'rb') as f:
            vocab = json.load(f)
    questions = []
    answers = []
    label_start_idx = []
    ...
    path_vocab = './vqa_preparedata/data/vocab_mimic_VQA.json'
    path_splits = './vqa_preparedata/data/splits_mimic_VQA.json'
...
def main():
    ...
    parser.set_defaults(save_coco_format=True, transform_dataset=True)
...
if __name__ == '__main__':
    nltk.data.path.append('./vqa_prepare/nltk_data')
    main()